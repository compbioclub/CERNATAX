...
class ceRNAxis():
    ...
    def __init__(self):
        ...
        self.ref_net = pd.read_csv(os.path.join(wdr, 'data', 'ceRNA_ref.csv'), index_col=0)
        
    
    def summarize_ref_net(self):
        ref_net = self.ref_net
        
        print('A total of {} miRNA-mRNA interaction and {} miRNA-lncRNA interaction'.format(
            ref_net[ref_net.type == 'miRNA-mRNA'].shape[0],
            ref_net[ref_net.type == 'miRNA-lncRNA'].shape[0],
        ))
        print(ref_net.type.value_counts())

    def find_ceRNA_axis_by_DEG_list(self, deg_miRNA, deg_ceRNA):
        ref_net = self.ref_net

        df = ref_net[ref_net.miRNA.isin(deg_miRNA) & ref_net.ceRNA.isin(deg_ceRNA)]
        ceRNA_df = df
    ...
    def expand_ceRNA_axis_by_loose_DEG(self, deg_strict_df, deg_df):
        ...
        df = ref_net[ref_net.miRNA.isin(axis_miRNA_list) & ref_net.ceRNA.isin(deg_ceRNA)]
